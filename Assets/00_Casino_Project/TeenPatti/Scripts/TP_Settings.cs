...
public class TP_Settings : MonoBehaviour
{
    public static TP_Settings Inst;
    ...
    public void Open_Manu()
    {
        ...
        iTween.MoveTo(this.gameObject, iTween.Hash("position", GameObject.Find("Table_BG").transform.position, "time", 0.3f, "easetype", iTween.EaseType.easeOutExpo));
    }
    ...
    public void Music_ON_OFF()
    {
        ...
        if (PlayerPrefs.GetInt("music").Equals(1))
        {
            ...
            PlayerPrefs.SetInt("music", 0);
            TP_SoundManager.Inst.BGAudio.mute = true;
        }
        else
        {
            IMG_MUSIC.sprite = Music_ON_Sprite;
            PlayerPrefs.SetInt("music", 1);
            TP_SoundManager.Inst.BGAudio.mute = false;
        }
    }

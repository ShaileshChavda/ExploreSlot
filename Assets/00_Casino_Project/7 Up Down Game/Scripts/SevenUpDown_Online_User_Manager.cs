...
public class SevenUpDown_Online_User_Manager : MonoBehaviour
{
    public static SevenUpDown_Online_User_Manager Inst;
    public SevenUpDown_PFB_OnlineUser PFB_Online_User;
    public RectTransform DataParent;
    internal List<SevenUpDown_PFB_OnlineUser> UserCellList;
    // Start is called before the first frame update
    void Start()
    {
        Inst = this;
        UserCellList = new List<SevenUpDown_PFB_OnlineUser>();
    }
    ...
    public IEnumerator SET_USER_LIST(JSONObject data)
    {
        ...
        for (int i = 0; i < data.GetField("user_joins").Count; i++)
        {
            ...
            {
                j++;
                SevenUpDown_PFB_OnlineUser cell = Instantiate(PFB_Online_User);
